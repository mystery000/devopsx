...
@click.option(
    ...
    default="openai",
    help="LLM to use.",
    type=click.Choice(["openai", "azure", "google", "local"]),
)
@click.option(
    "--model",
    default="gpt-4",
...
def _include_paths(msg: Message) -> Message:
    ...
    if append_msg and not msg.content.startswith("/ssh") and not msg.content.startswith("/shell") and not msg.content.startswith("/ps"):
